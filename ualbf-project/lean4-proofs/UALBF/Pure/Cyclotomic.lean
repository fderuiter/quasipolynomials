...
/--
  Algebraic bounding step for composite numbers.
  For $p \ge 3, q \ge 3, k \ge 1$ with $p \nmid q$, the cyclotomic ratio expands to dominate $p^k q$.
-/
lemma cyclotomic_composite_bound (p k q : ℕ) (hp : 3 ≤ p) (hk : 1 ≤ k) (hq : 3 ≤ q) (h_ndvd : ¬(p ∣ q)) :
    p ^ k * q * (2 ^ (p ^ k) - 1) < (2 ^ (p ^ (k - 1) * q) - 1) ^ (p - 1) := by
  sorry

/--
  Index bound for p = 2: Φ_n(2) > n for odd n ≥ 3.
  For odd `n ≥ 3`, `Φ_n(2) > n`.
-/
...
/--
  Cyclotomic evaluation exceeds the index.
