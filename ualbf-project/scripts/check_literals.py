    ...
    for root, _, files in os.walk(start_dir):
        for file in files:
            if file.endswith(".rs") and file != "manifest_constants.rs":
                rs_files.append(os.path.relpath(os.path.join(root, file), os.path.dirname(start_dir)))
    return rs_files

FILES_TO_CHECK = get_rs_files(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "rust-engine", "src"))
