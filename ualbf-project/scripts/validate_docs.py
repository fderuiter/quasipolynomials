...
def main():
    ...
    if unregistered:
        print("Error: The following documentation files are not registered in docs_manifest.json:", file=sys.stderr)
        for f in unregistered:
            print(f"  - {f}", file=sys.stderr)
        print("\nPlease add them to docs_manifest.json with their authority level ('authoritative' or 'informal').", file=sys.stderr)
        sys.exit(1)
            ...
            for f in pr_files:
                if f.endswith('.md'):
                    if f not in manifest:
                        print(f"Error: PR introduces a documentation file '{f}' not registered in docs_manifest.json.", file=sys.stderr)
                        print("Please add it to docs_manifest.json with its authority level.", file=sys.stderr)
                        sys.exit(1)
