    ...
    def test_duplicate_macro_halts_execution(self):
        ...
        with tempfile.TemporaryDirectory() as root_dir:
            ...
            with open(os.path.join(root_dir, "bounds_manifest.json"), "w") as f:
                json.dump({"omega_bounds": {"prasad_sunitha": {"proof_bound": 16, "engine_justified_gap": 0}, "hagis1982": {"proof_bound": 7, "engine_justified_gap": 0}}, "euler_ceiling": 100, "search_bounds": {"target_min_log10": {"value": 35}, "target_max_log10": {"value": 37}}}, f)
                
            with open(os.path.join(root_dir, "proof_manifest.json"), "w") as f:
                json.dump({
                    "theorems": [
                        {"name": "fermat_3", "status": "Verified", "checksum": "abc"},
                        {"name": "fermat.3", "status": "Verified", "checksum": "def"}
                    ]
                }, f)
            ...
            orig_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ingest_cert.py")
            ...
            try:
                ...
                with self.assertRaises(SystemExit) as cm:
                    exec(compile(source, script_path, "exec"), {"__file__": script_path, "__name__": "__main__"})
