...
includes an \texttt{\_ok} sentinel.  The
\texttt{ualbf\_compute\_sigma\_ok} export (mirroring the existing
\texttt{ualbf\_mod\_inverse\_ok}) verifies that
$\sigma(p^e) < 2^{128}$ \emph{before} the Rust engine reads the
truncated \texttt{\_lo}/\texttt{\_hi} halves.  A future extension
of the search bounds beyond~$10^{37}$ will be guarded against
modulo-truncated garbage by these sentinels.
