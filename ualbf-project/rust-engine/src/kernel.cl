...
inline RNS512 sub_rns(RNS512 a, RNS512 b) {
    RNS512 res;
    uint64_t borrow = 0;
    for(int i=0; i<8; i++){
...
inline RNS512 gcd_rns(RNS512 a, RNS512 b) {
...
inline RNS512 mont_mul(RNS512 a, RNS512 b, RNS512 m, uint64_t m0_prime) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        for (int j = 0; j < 8; ++j) {
            ...
            uint64_t hi = mul_hi(a.w[i], b.w[j]);
...
__kernel void pollard_rho(
    __global const Task* tasks,
    ...
    for(uint64_t c = 1; c < 40; ++c) {
        ...
        while(is_one(d)) {
            ...
            while (k < r && is_one(d)) {
                ...
                for(uint32_t j=0; j<batch; j++) {
                    Y = mont_add(mont_mul(Y, Y, n, t.m0_prime), C, n);
                    RNS512 diff = cmp(X, Y) > 0 ? sub_rns(X, Y) : sub_rns(Y, X);
        ...
        if (cmp(d, n) == 0) {
            while (true) {
                ...
                RNS512 diff = cmp(X_norm, YS_norm) > 0 ? sub_rns(X_norm, YS_norm) : sub_rns(YS_norm, X_norm);
                d = gcd_rns(diff, n);
                if (!is_one(d)) break;
            }
            if (cmp(d, n) != 0) {
...
typedef struct {
    ...
    bool do_verify;
} PrefixVerificationData;

__kernel void raycast_sieve(
    const RNS512 r_i,
    const RNS512 s_l,
    const uint64_t c_min,
    const uint64_t c_max,
    __global const Obstruction* obstructions,
    const uint32_t num_obstructions,
    __global volatile uint* bit_vector,
    __global uint32_t* valid_indices,
    __global volatile uint* valid_count,
    const uint8_t enable_diagnostics,
    const PrefixVerificationData prefix_data,
    const RNS512 z_max
) {
    uint id = get_global_id(0);
    ...
    if (!passed) {
        if (enable_diagnostics != 0) {
            ...
            atomic_or(&bit_vector[word_idx], 1 << bit_idx);
        }
    } else {
        uint idx = atomic_add(valid_count, 1);
