...
pub enum PruneReason {
    TargetBound,
    UnconditionalStarvation { max_allowed: usize, static_best_remaining: u128, lhs: Uint, rhs: Uint },
    OverflowKill { s_l_mul: Uint, n_l_mul: Uint },
    EulerCeiling { num: Uint, den: Uint, euler_num: Uint, euler_den: Uint },
    DynamicStarvation { dynamic_best_achievable_fp: u128, lhs: Uint, rhs: Uint },
    MinFactors { dynamic_min_factors: usize, curr_factors: usize, remaining_components: usize },
    Raycast,
}
        ...
        let handle = std::thread::spawn(move || {
            ...
            for event in receiver {
                ...
                match &event.reason {
                    ...
                    PruneReason::UnconditionalStarvation { max_allowed, static_best_remaining, lhs, rhs } => {
                    ...
                    PruneReason::EulerCeiling { num, den, euler_num, euler_den } => {
                    ...
                    PruneReason::DynamicStarvation { dynamic_best_achievable_fp, lhs, rhs } => {
                        ser_event.reason = "dynamic_starvation";
                        ser_event.dynamic_best_achievable_fp = Some(dynamic_best_achievable_fp.to_string());
                        ser_event.lhs = Some(lhs.to_string());
                        ser_event.rhs = Some(rhs.to_string());
                    }
                    PruneReason::MinFactors { dynamic_min_factors, curr_factors, remaining_components } => {
